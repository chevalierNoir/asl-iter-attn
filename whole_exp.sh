...
if [ $stage -eq 1 ];then
    ...
    for iter in ${iters[@]};do
        ...
        # resize prior
        if [ $iter -eq 0 ];then
            python -B make_map.py --img $iter_dir/prior_$iter --mat $iter_dir/priormat_$iter --csv $whole_csv --start $sid --end $eid || exit 1;
        else
        ...
        if [ $iter -eq ${iters[-1]} ];then
            python -B train.py --img $iter_dir/rgb_$iter --prior $iter_dir/priormat_$iter --csv $csv_dir --output $model_dir --pretrain $prt_conv --epoch ${epochs[$iter]} --conf $config --scaling --img_scale 1.0 0.8 1.2 --map_scale 13 10 15 || exit 1;            
        else
